...
async function seedDatabase() {
    ...
    const hackMaster = await Achievement_1.Achievement.create({
    ...
    const knowledgeSeeker = await Achievement_1.Achievement.create({
        title: 'Knowledge Seeker',
        description: 'Attend 3 or more academic seminars.',
        badgeImage: 'https://images.unsplash.com/photo-1579546929518-9e396f3cc809?w=100',
        xpBonus: 50
    });
    const communityLeader = await Achievement_1.Achievement.create({
        title: 'Eco Warrior',
    ...
    const coordinator = await User_1.User.create({
    ...
    // Additional Faculty
    const f1 = await User_1.User.create({
    ...
    const f2 = await User_1.User.create({
    ...
    await ClubMember_1.ClubMember.create({ clubId: acm.id, userId: student.id });
    await ClubMember_1.ClubMember.create({ clubId: ieee.id, userId: student.id });
    await ClubMember_1.ClubMember.create({ clubId: acm.id, userId: s1.id });
    await ClubMember_1.ClubMember.create({ clubId: sports.id, userId: s1.id });
    await ClubMember_1.ClubMember.create({ clubId: ieee.id, userId: s2.id });
