...
// Output file path
const outputPath = path_1.default.join(__dirname, '../../../prd_unisphere.pdf');
...
addParagraph('UniSphere implements 8 interactive sliding drawers in the user interface to simulate real-time university services and administrative dashboard options:');
...
addParagraph('Calculated in real-time inside the AI Service. Student interests (e.g. computer science, machine learning, sports) and event description details are parsed, stop-words are filtered, and frequency term vectors are generated. The system calculates the Cosine Similarity coefficient (the dot product of both vectors divided by their magnitudes) to rank event matches on the student dashboard, complete with justification strings.');
addSubheading('4.2 Attendance Regression Predictor');
addParagraph('Uses historical attributes (event categories, capacities, and day of week) to predict attendance rates and occupancy limits on the coordinator dashboard, identifying factors contributing to registration likelihood.');
addSubheading('4.3 Smart Schedule Suggestion');
addParagraph('Conflict optimization suggestions identify optimal dates and times with the lowest potential conflict index based on current calendars and coordinator workloads.');
...
addParagraph('Built with Express (Node.js/TypeScript) utilizing Sequelize ORM. The database connects to PostgreSQL in production and falls back to zero-config SQLite (`unisphere.sqlite`) in dev mode for offline ease of use.');
addSubheading('5.2 Caching Strategy (Redis)');
addParagraph('Event schedules are cached in Redis to minimize database queries. Cache is invalidated on event creation, deletion, or admin approval. Fallback to in-memory bypass is active if Redis is offline.');
...
addParagraph('Configured unit tests in package.json (run via npm test) which assert JWT tokenization, Cosine Similarity calculations, and prediction regression models.');
addSubheading('6.2 CI/CD Pipelines');
addParagraph('Provided a GitHub Actions CI/CD configuration (.github/workflows/ci.yml) which automatically triggers dependencies installations, TypeScript compiler builds, and unit test suites on pushes and pull requests.');
addSubheading('6.3 Cloud & Multi-Container Blueprints');
addParagraph('Provided docker-compose.yml files linking PostgreSQL database, Redis cache, Node backend, and Nginx frontend client, alongside Render render.yaml blueprints and Vercel routing configs.');
// End the document
doc.end();
